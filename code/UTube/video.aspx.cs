    ...
    public partial class video : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                imhSmall.ImageUrl = @"\Data\Account\" + Session["User"].ToString() + @"\DP\" + fi[0].Name;

                string watch = (string)Request.QueryString["watch"];
                if (watch == null)
                    Response.Redirect("~/account.aspx");

                tbVideos.Text = DBWorks.GetFeeds("", "video");
                ...
                //Adding to history
                DBWorks.AddToHistory(Session["User"].ToString(), upID);
